// Constants

class Constants {
  // Bibles
  static const String kjvbDbname = "kjv.db"; // 1
  static const String clemDbname = "clem.db"; // 2
  static const String cpdvDbname = "cpdv.db"; //3
  static const String nvulDbname = "nvul.db"; // 4
  //static const String af53Dbname = "dba_afr53_1.db"; // 5
  //static const String dn33Dbname = "dba_dn1933_1.db"; // 6
  static const String ukjvDbname = "ukjv.db"; // 7
  static const String webbDbname = "webbe.db"; // 8
  //static const String af83Dbname = "ndba_afr83.db"; // 9
  static const String asvbDbname = "asv.db"; // 10

  // Bookmarks
  static const String bmksDbname = "ndba_bkmarks.db";

  // Highlights
  static const String hltsDbname = "ndba_hlights.db";

  // Language Key
  //static const String lkeyDbname = "dba_lkey_1.db";

  // Version Key
  static const String vkeyCvsName = "vkey.cvs";

  // Notes
  static const String notesDbname = "notes.db";

  // Dictionary
  static const String dictDbname = "dict.db";
}
